...
- **API** is ~154 `route.ts` handlers under `src/app/api/**`. There is **no shared auth helper**: every route inlines `getServerSession(authOptions)` + a role/ownership check (~345 call sites). `src/middleware.ts` routes by host (bare domain → www; any other subdomain, the retired showcase city hosts included → the same path on www; rules in `src/lib/showcase-hosts.ts`), marks a path that can only be a professional's page (`www.jechemine.ca/<slug>`, `src/app/[proSlug]`) and injects `x-pathname` — it does **not** enforce auth.
