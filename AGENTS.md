...
- **API** is ~154 `route.ts` handlers under `src/app/api/**`. There is **no shared auth helper**: every route inlines `getServerSession(authOptions)` + a role/ownership check (~345 call sites). `src/middleware.ts` routes by host (bare domain → www; a showcase city host `psy<city>.jechemine.ca` → `src/app/showcase/[cityKey]/…`, rules in `src/lib/showcase-hosts.ts`) and injects `x-pathname` — it does **not** enforce auth.
