...
/** An admin sets or removes the photo of a professional's draft page. */
...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  const result = await setShowcasePhoto({ userId, fileId: stored.fileId, actor: "admin" });
...
export async function DELETE(_req: NextRequest, { params }: Params) {
  ...
  if (gate.error) return gate.error;
  const { userId } = await params;
  const result = await setShowcasePhoto({ userId, fileId: null, actor: "admin" });
  return respondShowcase(result, (value) => value);
}
