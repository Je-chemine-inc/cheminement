...
export async function PUT(req: NextRequest) {
  try {
    ...
    // Re-enabling intake must UNBLOCK the queue: demandes that piled up unmatched
    // in the general pool while intake was OFF are re-offered to this pro (shared
    // helper, also used by the admin endpoint). acceptingNewClients is the master
    // gate; emergency only matters while it is ON.
