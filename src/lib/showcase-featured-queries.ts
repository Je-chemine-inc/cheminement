...
import {
  ...
  type FeaturedLocale,
  type FeaturedPageSource,
  type FeaturedProfessional,
  type FeaturedProfileSource,
} from "@/lib/showcase-featured";

/** The reads behind « Quelques-uns de nos professionnels ». Only the fields the section may use are loaded. */
export async function loadFeaturedProfessionals(locale: FeaturedLocale): Promise<FeaturedProfessional[]> {
  ...
  const pages = showcaseOn
    ? (
        (await ShowcasePage.find({ userId: { $in: ids }, status: "published" })
          ...
          .lean()) as unknown as (FeaturedPageSource & { cityKey: string })[]
      ).filter((page) => isShowcaseCityKey(page.cityKey))
    : [];
  return buildFeaturedProfessionals({
    locale,
    users,
    profiles: profiles as unknown as FeaturedProfileSource[],
    pages,
    showcaseOn,
  });
}
