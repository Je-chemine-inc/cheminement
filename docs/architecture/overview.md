...
- **Thin CI quality gate** — `.github/workflows/deploy-whc.yml` runs `pnpm test` then `next build` (either failing blocks the deploy), but not ESLint or `tsc --noEmit`, and `next build` skips type errors in spec files; a green run deploys straight to production. See [debt-map](../quality/debt-map.md).
