...
> **2026-09-15 — no longer used by the pages.** The owner moved each professional's page to `www.jechemine.ca/<slug>` and removed the city pages ([ADR-0003 amendment](../architecture/decisions/0003-per-city-showcase-hosts.md#amendment--2026-09-15-one-address-on-www)). Everything below stays in place for a later use: once deployed, a city host answers with a 307 to the same path on www.

The showcase pages live on one host per Quebec city (`psymascouche.jechemine.ca/sassi`), served by the same app: `src/middleware.ts` rewrites a city host to `src/app/showcase/[cityKey]/…` (rules in `src/lib/showcase-hosts.ts`; why: [ADR-0003](../architecture/decisions/0003-per-city-showcase-hosts.md)). The code is inert until this section is done, and the pages stay dark until an admin turns `showcaseEnabled` on. **Adding a city later is a code change only** (`src/data/canadaCities.ts`) — nothing on the server. Since 2026-09-13 the list holds all 234 official « Ville » of Quebec (MAMH Répertoire des municipalités, Données Québec, CC BY 4.0 — credited on `/psy`) plus Rawdon, Boischatel, Stoneham-et-Tewkesbury and Les Îles-de-la-Madeleine: 238 hosts. The 15 boroughs and former cities already listed (Verdun, Hull, Chicoutimi…) carry `partOf` and get no host until the owner decides. Never rename or remove a city once its host is live: `showcase-cities.spec.ts` pins every host label.
